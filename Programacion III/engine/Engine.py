...
# Definir las 5 materias del primer semestre
primer_semestre = [
    "Humanidades_I",
    "Deportes_I",
    "Matematicas_Fundamentales",
    "Desarrollo_pensamiento_logico",
    "Introduccion_informatica"
]

primer_semestre_ids = [idx[materia] + 1 for materia in primer_semestre]  # +1 porque MiniZinc usa 1..n

print(f"\n📚 Materias del primer semestre (requeridas):")
for materia in primer_semestre:
    print(f"   • {materia}")
...
with open(SALIDA_DZN, "w", encoding="utf-8") as f:
    ...
    f.write("% Variables para la restricción del primer semestre\n")
    f.write(f"num_primer_semestre = {len(primer_semestre)};\n")
    f.write("primer_semestre_ids = [\n")
    for pid in primer_semestre_ids:
        f.write(f"  {pid},\n")
    f.write("];\n\n")
